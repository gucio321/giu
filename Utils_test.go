...
func Test_ToVec4(t *testing.T) {
	tests := []struct {
		name     string
		source   color.Color
		expected cimgui.ImVec4
	}{
		{
			name:     "Red - RGBA",
			source:   &color.RGBA{R: 255, G: 0, B: 0, A: 255},
			expected: cimgui.ImVec4{X: 1, Y: 0, Z: 0, W: 1},
		},
		{
			name:     "Purple - RGBA",
			source:   &color.RGBA{R: 158, G: 0, B: 173, A: 255},
			expected: cimgui.ImVec4{X: 0.61960787, Y: 0, Z: 0.6784314, W: 1},
		},
		{
			name:     "Purple - CMYK",
			source:   &color.CMYK{C: 22, M: 255, Y: 0, K: 82},
			expected: cimgui.ImVec4{X: 0.6198978, Y: 0, Z: 0.6784314, W: 1},
		},
	}
...
func Test_ToVec2(t *testing.T) {
	tests := []struct {
		name     string
		source   image.Point
		expected cimgui.ImVec2
	}{
		{"Point 0,0", image.Pt(0, 0), cimgui.ImVec2{X: 0, Y: 0}},
		{"Random point 1", image.Pt(80, 209), cimgui.ImVec2{X: 80, Y: 209}},
		{"Random point 2", image.Pt(200, 128), cimgui.ImVec2{X: 200, Y: 128}},
	}
...
func Test_Vec4ToRGBA(t *testing.T) {
	tests := []struct {
		name     string
		source   cimgui.ImVec4
		expected color.RGBA
	}{
		{
			name:     "Red",
			source:   cimgui.ImVec4{X: 1, Y: 0, Z: 0, W: 1},
			expected: color.RGBA{R: 255, G: 0, B: 0, A: 255},
		},
		{
			name:     "Red - with 20% alpha",
			source:   cimgui.ImVec4{X: 1, Y: 0, Z: 0, W: 0.2},
			expected: color.RGBA{R: 255, G: 0, B: 0, A: 51},
		},
		{
			name:     "Purple - RGBA",
			source:   cimgui.ImVec4{X: 0.61960787, Y: 0, Z: 0.6784314, W: 1},
