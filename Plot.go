...
func (p *PlotCanvasWidget) Build() {
	if len(p.plots) > 0 {
		cimgui.Plot_SetupAxesLimitsV(p.xMin, p.xMax, p.yMin, p.yMax, cimgui.ImPlotCond(p.axisLimitCondition))
		cimgui.Plot_SetupAxesV(
			Context.FontAtlas.RegisterString(p.xLabel),
			Context.FontAtlas.RegisterString(p.yLabel),
			cimgui.ImPlotAxisFlags(p.xFlags),
			cimgui.ImPlotAxisFlags(p.yFlags),
			//cimgui.ImPlotAxisFlags(p.y2Flags),
			//cimgui.ImPlotAxisFlags(p.y3Flags), Context.FontAtlas.RegisterString(p.y2Label), Context.FontAtlas.RegisterString(p.y3Label),
		)

		// TODO: figure out what is it
		//if len(p.xTicksValue) > 0 {
		//	cimgui.ImPlotSetNextPlotTicksX(p.xTicksValue, p.xTicksLabel, p.xTicksShowDefault)
		//}
		...
		if cimgui.Plot_BeginPlotV(
			Context.FontAtlas.RegisterString(p.title),
			//Context.FontAtlas.RegisterString(p.xLabel),
			//Context.FontAtlas.RegisterString(p.yLabel),
			ToVec2(image.Pt(p.width, p.height)),
			cimgui.ImPlotFlags(p.flags),
			//imgui.ImPlotAxisFlags(p.xFlags),
			//cimgui.ImPlotAxisFlags(p.yFlags), imgui.ImPlotAxisFlags(p.y2Flags),
			//cimgui.ImPlotAxisFlags(p.y3Flags), Context.FontAtlas.RegisterString(p.y2Label), Context.FontAtlas.RegisterString(p.y3Label),
		) {
...
func Bar(title string, data []float32) *BarPlot {
	return &BarPlot{
		title:  title,
		data:   data,
		width:  0.2,
...
func (p *BarPlot) Offset(offset int32) *BarPlot {
	p.offset = offset
	return p
}
...
func (p *BarPlot) Plot() {
	cimgui.Plot_PlotBars_FloatPtrIntV(p.title, p.data, int32(len(p.data)), p.width, p.shift, cimgui.ImPlotBarsFlags_None, p.offset, 32)
}
...
func BarH(title string, data []float32) *BarHPlot {
	return &BarHPlot{
		title:  title,
		data:   data,
		height: 0.2,
...
func (p *BarHPlot) Plot() {
	cimgui.Plot_PlotBars_FloatPtrFloatPtrV(
		Context.FontAtlas.RegisterString(p.title),
		[]float32{},
		p.data,
		int32(len(p.data)),
		p.height,
		cimgui.ImPlotBarsFlags_None,
		//p.shift, // TODO
		p.offset,
		32,
	)
...
func Line(title string, values []float32) *LinePlot {
	return &LinePlot{
		title:  title,
		values: values,
		xScale: 1,
...
func (p *LinePlot) Offset(offset int32) *LinePlot {
	p.offset = offset
	return p
}
...
func (p *LinePlot) Plot() {
	cimgui.Plot_SetAxis(cimgui.ImAxis(p.yAxis))
	cimgui.Plot_PlotLine_FloatPtrIntV(
		Context.FontAtlas.RegisterString(p.title),
		p.values,
		int32(len(p.values)),
		p.xScale,
		p.x0,
		cimgui.ImPlotLineFlags_None,
		p.offset,
		32,
	)
}
...
func LineXY(title string, xvalues, yvalues []float32) *LineXYPlot {
	return &LineXYPlot{
		title:  title,
		xs:     xvalues,
		ys:     yvalues,
...
func (p *LineXYPlot) Offset(offset int32) *LineXYPlot {
	p.offset = offset
	return p
}
...
func (p *LineXYPlot) Plot() {
	cimgui.Plot_SetAxis(cimgui.ImAxis(p.yAxis))
	cimgui.Plot_PlotLine_FloatPtrFloatPtrV(
		Context.FontAtlas.RegisterString(p.title),
		p.xs,
		p.ys,
		int32(len(p.xs)),
		cimgui.ImPlotInfLinesFlags_None,
		p.offset,
		32,
	)
}
